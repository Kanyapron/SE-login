...
type (
	...
	signUp struct {
		...
		UserRoleID  uint    `json:"userrole_id"`
 ...
 func SignIn(c *gin.Context) {
	...
	if err := config.DB().Raw("SELECT * FROM users WHERE email = ?", payload.Username).Scan(&user).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
